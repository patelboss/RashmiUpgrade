...
#
# NOTE: This module is not imported anywhere in the project right now
# (it's commented out at the top of utils.py) and has one more pre-existing
# issue beyond the syntax error just fixed here: it uses the synchronous
# `pymongo.MongoClient`, but every function `await`s its collection calls
# as if it were Motor's async client — that will raise a TypeError the
# moment this module is actually wired in. Left as-is since fixing it
# would mean guessing at intended behavior (swap to Motor? drop the
# awaits?) rather than a mechanical fix — flagging it for your call.
