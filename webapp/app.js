...
el.textContent = text;
el.style.cssText = "position:fixed; bottom:80px; left:50%; transform:translateX(-50%); background:var(--accent); color:var(--btn-text); padding:8px 18px; border-radius:100px; font-size:.82rem; font-weight:600; z-index:999; pointer-events:none; opacity:0; transition:opacity .2s;";
...
card.innerHTML = "<div class="file-thumb ${thumbClass(type)}">${typeEmoji(type)}</div> <div class="file-info"> <div class="file-name" title="${escHtml(file.file_name || '')}">${escHtml(file.file_name || 'Untitled')}</div> <div class="file-meta"> <span class="file-type-badge ${typeBadgeClass(type)}">${type || 'file'}</span> <span class="file-size">${fmt(file.file_size)}</span> </div> <div class="file-caption">${escHtml(file.file_name || 'Untitled')}</div> </div> <svg class="file-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>";
...
sheetContent.innerHTML = "<div class="sheet-file-name">${typeEmoji(type)} ${escHtml(file.file_name || 'Untitled')}</div> <div class="sheet-meta"> <div class="sheet-row"> <span class="sheet-row-key">Type</span> <span class="sheet-row-val">${type || 'file'}</span> </div> <div class="sheet-row"> <span class="sheet-row-key">Size</span> <span class="sheet-row-val">${fmt(file.file_size)}</span> </div> ${file.mime_type ?"
<div class="sheet-row">
<span class="sheet-row-key">MIME</span>
<span class="sheet-row-val" style="font-size:.72rem">${escHtml(file.mime_type)}</span>
</div>": ''} </div> ${file.caption ?"<div class="sheet-caption">${escHtml(file.caption)}</div>": ''} <div class="sheet-actions"> <button class="btn-primary" onclick="getFile()">📥 Get File</button> <button class="btn-secondary" onclick="closeSheet()">Close</button> </div>";
